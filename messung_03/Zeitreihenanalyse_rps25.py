...
def perform_stft(zeitreihe, segmente, rps, brett):
    ...
    f, t, Sxx = signal.stft(zeitreihe, 1/(1/25), nperseg=segmente)
...
df = pd.read_csv("Testdaten/rps25_synch.txt", sep=';', names=['ProfilNR','PNR','X','Y','Z','I','t'])
...
bretter = [1675, 2400, 5600, 6800, 8940, 8050]

prot(f"Interessante Punkte: {bretter} \n\n")

# Ein Profil mit allen Bretterpunkten als Übersicht plotten
base = plt.plot(df['Y'][0:10000],df['Z'][0:10000], label='Beispielprofil')
...
for brett in bretter:
    ...
    f, time, Sxx = perform_stft(zeitreihe, 25, rps, brett)
    ...
    startpunkt, endpunkt = 300, 630 # 300,900
    anzahl_koeffizienten = 321 # (ungerade)  # 501
